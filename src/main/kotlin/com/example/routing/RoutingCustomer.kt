    ...
    route("/Customers") {
        post("/ExclusiveOffers") {
            try {
                ...
                exclusive = product.filter { it.item_category_name == "exclusive" }
                apiListResponse(
        ...
        get("/HomeCategoryWiseProducts") {
            try {
                ...
                val cat1: List<HomeProducts> = product.filter { mainCategory ->
                    allCategory.lastOrNull()?.subCategoryList?.getOrNull(0)?.name == mainCategory.item_category_name.toString()
                }.take(4)
                println("result iss ${cat1.size}")
                val cat2: List<HomeProducts> = product.filter { mainCategory ->
                    "milk" == mainCategory.item_category_name.toString()
                }.take(4)
...
//        }
        post("/BestSelling") {
            try {
                val product = db.getHomeAllProducts()
                val best = product.filter { it.item_category_name == "best" }
                apiListResponse(
